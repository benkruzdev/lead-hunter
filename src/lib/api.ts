...
//
// Source-of-truth map:
//   Package catalog    → GET /api/billing/packages   (public, no auth)
//   Order history      → GET /api/billing/orders     (requireAuth)
//   User credit balance → GET /api/account           (requireAuth + requireActiveLifecycle)
//   Credit ledger/history → GET /api/credits/history (requireAuth)

/**
 * CreditPackage as returned by the public /api/billing/packages endpoint.
 * Backend returns raw snake_case; localisation is handled client-side.
 * fields is_featured / description / features exist from migration 020 onward.
 */
export interface CreditPackage {
    ...
    price_usd: number;
    description: string | null;
    /** JSONB array of feature bullet-point strings */
...
/**
 * BillingOrder as returned by GET /api/billing/orders.
 * Source of truth for order history. Credit balance is NOT read from here —
 * use GET /api/account for the authoritative credit balance.
 * 
 * Note: `failure_code` and `last_payment_event_at` are intentionally omitted
 * from this public contract to keep them as admin-only diagnostic fields.
 */
export interface BillingOrder {
    ...
    /** Checkout redirect URL generated by the gateway (may be null/expired) */
    checkoutUrl: string | null;
    /** Opaque provider reference string (e.g. PayTR merchant_oid) */
    providerReference: string | null;
    /** Human-readable failure reason (set on payment failure) */
    failureReason: string | null;
    createdAt: string;
}

export async function getOrders(): Promise<{ orders: BillingOrder[] }> {
